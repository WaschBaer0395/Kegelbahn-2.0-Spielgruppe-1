...
export default function DistanceBar({ }: DistanceBarProps) {
    ...
    useEffect(() => {
        ...
        const newPlayerPositions = game.getPlayers().map(player => {
            return (((player.getTotalScore() * game.getMultiplier()) + 30) / (game.getMultiplier() * game.getMaxScore())) * 93;
        });
    ...
    return (
        <div className="beam-chart">
            ...
            {game.getPlayers().map((player, index) => (
                <div key={player.id} className="marker-pin" style={{ left: `${playerPositions[index]}%`, zIndex: index , transition: `left ${import.meta.env.VITE_ANIMATIONSPEED}s ease`}}>
